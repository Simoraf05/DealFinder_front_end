...
const UpadatProfile = () => {
  ...
  if (image.image && image.image !== data.profile_picture) {
    payload.profile_picture = image.image;
  }
  if (adresse !== data.location) {
    payload.location = adresse;
  }
    axios.put(`http://127.0.0.1:8000/api/updateProfile/${data.id}`, {payload})
      .then((response) => {
