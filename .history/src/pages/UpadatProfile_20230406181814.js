  ...
  const UpadatProfile = () => {
    ...
    dataToUpdate._method="PUT"
    const handleUpdate = async (e) => {
      e.preventDefault();
      /*axios.post(`http://127.0.0.1:8000/api/updateProfile/${data.id}`,dataToUpdate)
        .then((response) => {
        ...
        try {
          const response = await fetch(`http://127.0.0.1:8000/api/updateProfile/${data.id}`, {
            method: 'POST',
            body: formData
          });
