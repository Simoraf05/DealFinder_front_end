...
const Home = () => {
  ...
  async function Load() {
    const result = dataArray.map((item) => {
      const categoryProducts = products.filter((product) => product.category === item[1]);
      return { category: item[1], productCount: categoryProducts.length };
    });
    setNumberOfProductsInCtg(result);
  }
  ...
  products.map((item) => {
    const imagePath = Object.keys(item.path)[0];
            ...
            <div className='pro__container'>
              {
                products.map((item) => (
                  <Link to='/productD' state={item} className='pro'>
                    <img src={`http://localhost:8000/` + item.path[0]} />
                    <div className='des__'>
