...
const Home = () => {
  ...
  const [numberOfProductsInCtg,setNumberOfProductsInCtg] = useState([{key:'value',key:'value'}])
  const numberOfProductsInCtg = Array.from(Array(2), () => ({
    key: 'value',
  }));
  useEffect(()=>{
    dataArray.map((item)=>{
      const fetchData = () => {
        try {
          ...
          .then(response=>{
            ...
            setNumberOfProductsInCtg({product_count:response.data.product_count,category:response.data.category})
          })
