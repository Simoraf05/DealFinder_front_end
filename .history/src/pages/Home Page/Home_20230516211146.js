...
const Home = () => {
  ...
  useEffect(()=>{
    dataArray.map((item)=>{
      const fetchData = () => {
        try {
          axios.post('http://127.0.0.1:8000/api/productsInCategory',{category:'[{"key":"category","value":"Equipement pour Bébé et Enfant","description":null,"type":"text","enabled":true,"equals":true}]'})
          .then(response=>{
