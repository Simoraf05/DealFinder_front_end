...
const Home = () => {
    ...
    const fetchData = async () => {
      try {
        for (const item of dataArray) {
          const response = await axios.post('http://127.0.0.1:8000/api/productsInCategory', { category: item[1] });
  
          if (response.data.length === 0) {
            continue; // Skip current iteration if response data is empty
          }
  ...
  numberOfProductsInCtg.map((item, index) => {
    const category = item.category || 'N/A';
    const productCount = item.product_count || 'N/A';
    console.log(`Item ${index}: Category - ${category}, Product Count - ${productCount}`);
  });
              ...
              dataArray.map((item2) => (
                dataArray.map((item)=>(
                  ...
                  <div className='fea__text'>
                    <h4>{item[1]}</h4>
                    <p>{
                      item2[0]['category'] === item[1] ?
                      item2[0]['product_count'] : 0
                      } items</p>
                  </div>
                  <div className='arrow'>
