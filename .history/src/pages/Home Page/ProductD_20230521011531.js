...
const ProductD = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post('http://127.0.0.1:8000/api/getComments', { id: location.state.id });
        ...
        <div>
            <section id='productdetail__'>
                <div className='single__pro__image'>
                    <img src={`http://localhost:8000/` + location.state.item.path[0]} width='70%' id='MainImg__' />
                </div>
                                ...
                                <div className="card p-3 comment__detail">
                                    <div className="d-flex justify-content-between align-items-center">
                                        <div className="user__ d-flex flex-row align-items-center">
                                            <img src={`http://localhost:8000/` + item.profile_picture} width="30" className="user-img rounded-circle mr-2 m-3" />
                                            <span><small className="font-weight-bold text-primary">{item.name}</small> <small className="font-weight-bold">{item.comment}</small></span>
                                        </div>
