...
const Home = () => {
    ...
    const fetchData = async () => {
      try {
        for (const item of dataArray) {
          const response = await axios.post('http://127.0.0.1:8000/api/productsInCategory', { category: item.category });
  ...
  numberOfProductsInCtg.map((item)=>{
    console.log(item[0].product_countt)
  })
