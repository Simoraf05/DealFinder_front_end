...
const FiltringProducts = () => {
    ...
    const dataFetch=(e)=>{
        e.preventDefault()
        axios.get('http://127.0.0.1:8000/api/filtring',title)
        .then(res=>{
            console.log(res.data.searchByTitle)
        })
    }
            ...
            <div class="row" id="search">
                <form id="search-form" onSubmit={dataFetch} >
                    <div class="form-group col-xs-9">
                        <input class="form-control" name='title' onChange={e=>{setTitle(e.target.value)}} type="search" placeholder="Search" />
                    </div>
