...
const MesCommandes = () => {
  ...
  useEffect(() => {
      axios.post('http://127.0.0.1:8000/api/getCommande',{buyer_id:currentUser.id})
        .then(response =>{
          ...
          console.log(response.data.mesC)
        })
        .catch(error => {
          console.log(error)
        });
  ...
  const updateOfferPrice=async(e)=>{
    e.preventDefault()
    try const response = await fetch(`http://127.0.0.1:8000/api/updatingOfferPrice/${commande.offer_id}`)
                              ...
                              <div class="col-md-3 col-lg-2 col-xl-2 offset-lg-1">
                                {
                                    ...
                                    <button type="button" class="btn btn-success">update offer price</button>
                                    :
