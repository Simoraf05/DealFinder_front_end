...
const Cart = () => {
  ...
  const [price,setPriceOffer] = useState([])
          ...
          <tbody>
            {
              cart.map((item)=>(
                ...
                <td>{item.title}</td>
                <td>${item.price}</td>
                <td><input type='number' min={item.price} onChange={(e) =>setP} defaultValue={item.price}/></td>
                <td><button onClick={()=>{addOffer({listing_id:item.id,price:priceOffer[index],seller:item.email})}} >Add</button></td>
