...
const Home = () => {
  ...
  useEffect(() => {
    const checkStock = async (productId) => {
      try {
        const response = await axios.get('http://127.0.0.1:8000/api/checkStock', { params: { product_id: productId } });
        setStockStatus(prevState => ({ ...prevState, [productId]: response.data.Product }));
      } catch (error) {
        console.log(error);
      }
    };
              ...
              <div class="row">
                {
                  products.map((item)=>(
                    ...
                    {stockStatus[item.product.id] !== undefined && (
                      <p>{stockStatus[item.product.id] ? 'In stock' : 'Out of stock'}</p>
                    )}
