...
const MesOff = () => {
    ...
    /*useEffect(() => {
      axios.post('http://127.0.0.1:8000/api/getStatusUpdated',{id:offer.offer_id})
      .then(response =>{
    ...
    const updateOfferStatus=(offer_id,idP)=>{

      axios.put(`http://127.0.0.1:8000/api/updatingProductStatus/${idP}`,{status:status})
      .then(res=>{
        console.log(res.data.offer.statusUpdated)
      })

      if(status === 'accepted'){
        axios.put(`http://127.0.0.1:8000/api/updatingOfferStatus/${offer_id}`,{status:1})
        .then(res=>{
