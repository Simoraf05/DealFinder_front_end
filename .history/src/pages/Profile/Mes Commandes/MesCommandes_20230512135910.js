...
const MesCommandes = () => {
  ...
  const updateOfferPrice=(e)=>{
    ...
    try{
      const response = axios.post(`http://127.0.0.1:8000/api/updatingOfferPrice/${props.state}`, {price:newPrice});
  ...
  return (
    <div>
      ...
      <div className='row'>
      <section id='table__' className="col-sm-4">
        <table >
          <thead>
            <tr>
              <td>Image</td>
          ...
          <tbody>
            {
              commande.map((item) => (
                <tr>
                  ...
                  <td>{item.title}</td>
                  <td>${item.price}</td>
                  <td><input type='number' onChange={e=>{setNewPrice(e.target.value)}} defaultValue={item.offer_price} /></td>
                  <td>${item.status}</td>
                  {
