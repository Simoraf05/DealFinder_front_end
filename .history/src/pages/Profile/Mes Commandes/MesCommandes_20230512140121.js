...
const MesCommandes = () => {
  ...
  useEffect(() => {
    axios.post('http://127.0.0.1:8000/api/getCommande', {
  ...
  const updateOfferPrice = (offer_id) => {
    ...
    try {
      const response = axios.post(`http://127.0.0.1:8000/api/updatingOfferPrice/${offer_id}`, {
        price: newPrice
      });
  ...
  return ( <
    ...
    h4 > #Cart < /h4> <
    h2 > Read all
    case studlies about our services! < /h2> <
    /section> <
    div className = 'row' >
    <
    section id = 'table__'
