...
const MesAnn = () => {
  ...
  const editeProduct = ({ 
    ...
    toast(<EditProduct items={{title:title,description:description,price:price,shipping_options:shipping_options}}></EditProduct>
      , {
        position: "top-center",
        autoClose: 5000,
            ...
            <tbody>
              {
                myProducts.map((item) => (
                  <tr>
                    <td><img src={`http://localhost:8000/` + item.item.path[0]} /></td>
                    <td>{item.title}</td>
                    <td>${item.price}</td>
                    <td>
                      <button class="btn btn-outline-success btn-sm btn__edit" onClick={() => {
                        editeProduct({
                          id: item.id,
                          title: item.title,
